...
const EarnPointsView = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (profileError || !profile) {
          ...
          await supabase.from('notifications').insert([
            {
              ...
              message:
                "We're thrilled to have you on board! Explore powerful tools to boost productivity.",
              is_read: false,
            },
          ])
        }
        ...
        setStreak(profile.streak || 0) // Ensure we start from the DB value
  ...
  const handleClaimPoints = async () => {
    ...
    try {
      ...
      const newStreak = streak + 1 // Increment the streak count

      const { error } = await supabase.from('profiles').upsert({
        id: user.id,
        points: newPoints,
        last_claim: now,
        streak: newStreak, // Save the incremented streak
      })
      ...
      await supabase.from('notifications').insert([
        {
          ...
          message: `Successfully claimed 5 Flowva points. Your new balance is ${newPoints} pts and your streak is now ${newStreak} days!`,
          is_read: false,
        },
      ])

      setPoints(newPoints)
      setStreak(newStreak) // Update local state so UI reflects change
      setCanClaim(false)
